...
public class ClientCredentialsTokenRequestHandler extends AbstractTokenRequestHandler
{
    ...
    public void refreshAccessToken() throws MuleException
    {
        try
        {
            final DefaultMuleEvent accessTokenEvent = new DefaultMuleEvent(new DefaultMessageExecutionContext(getMuleContext().getUniqueIdString(), null),
                    MuleMessage.builder().nullPayload().build(), MessageExchangePattern.REQUEST_RESPONSE, new Flow("test", getMuleContext()));
