...
public class AsynchronousUntilSuccessfulProcessingStrategyTestCase extends AbstractMuleTestCase
{
    ...
    public void alwaysFailUsingFailureExpression() throws Exception
    {
        ...
        verify(mockEvent.getFlowConstruct().getExceptionListener(), times(1)).handleException(argThat(new ArgumentMatcher<MessagingException>()
        {
            @Override
            public boolean matches(Object item)
            {
                return item instanceof MessagingException && EXPECTED_FAILURE_MSG.equals(((MessagingException) item).getCauseException().getMessage());
            }
        }), eq(mockEvent));
        ...
        verify(mockEvent.getFlowConstruct().getExceptionListener(), times(1)).handleException(argThat(new ArgumentMatcher<MessagingException>()
        {
            ...
            public boolean matches(Object item)
            {
                return item instanceof MessagingException
                       ...
                       && (((MessagingException) item).getCauseException()).getMessage().contains("until-successful retries exhausted. Last exception message was: " + EXPECTED_FAILURE_MSG);
            }
        }), eq(mockEvent));
        verify(mockDLQ, never()).process(any(MuleEvent.class));
    }
    ...
    public void alwaysFailUsingFailureExpressionDLQ() throws Exception
    {
        ...
        verify(mockEvent.getFlowConstruct().getExceptionListener(), never()).handleException(any(MessagingException.class), any(MuleEvent.class));
    ...
    public void successfulExecution() throws Exception
    {
        ...
        verify(mockEvent.getFlowConstruct().getExceptionListener(), never()).handleException(any(MessagingException.class), eq(mockEvent));
    }
    ...
    {
        when(mockEvent.getFlowConstruct().getExceptionListener().handleException(any(MessagingException.class), any(MuleEvent.class))).thenAnswer(invocationOnMock ->
        {
