  ...
  protected void initWorkManagers() throws MuleException {
    ...
    if (dispatcherWorkManager.get() == null) {
      ...
      WorkManager newWorkManager = dispatcherThreadingProfile.createWorkManager(
                                                                                threadPrefix, muleContext.getConfiguration()
                                                                                    .getShutdownTimeout());
  ...
  protected MessageDispatcher borrowDispatcher(OutboundEndpoint endpoint) throws MuleException {
    if (!isStarted()) {
      throw new LifecycleException(TransportCoreMessages.lifecycleErrorCannotUseConnector(getName(),
                                                                                          lifecycleManager.getCurrentPhase()),
                                   this);
    }
          ...
          if (validateConnections && context.getLastFailure() instanceof EndpointConnectException) {
            ...
            if (!failed.validateConnection(context).isOk()) {
              throw new EndpointConnectException(
                                                 MessageFactory.createStaticMessage("Still unable to connect to resource "
  ...
  protected void handleWorkException(WorkEvent event, String type) {
    ...
    if (e instanceof MessagingException) {
      ...
      muleContext.getExceptionListener().handleException(
                                                         new MuleRuntimeException(TransportCoreMessages
