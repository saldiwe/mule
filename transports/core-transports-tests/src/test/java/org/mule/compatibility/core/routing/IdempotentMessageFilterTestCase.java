...
public class IdempotentMessageFilterTestCase extends AbstractMuleContextEndpointTestCase
{
    ...
    public void testIdempotentReceiver() throws Exception
    {
        ...
        DefaultMuleEvent event = new DefaultMuleEvent(new DefaultMessageExecutionContext(muleContext.getUniqueIdString(), null), okMessage, getTestFlow(), session);
        ...
        event = new DefaultMuleEvent(new DefaultMessageExecutionContext(muleContext.getUniqueIdString(), null), okMessage, getTestFlow(), session);
        DefaultMuleEventEndpointUtils.populateFieldsFromInboundEndpoint(event, endpoint1);
